...
from Computer_Security_Project_Secure.forms import CreateUserForm, UserAccountForm, LoginForm
from django.core.mail import send_mail

from djangoProject1.settings import get_custom_user_model
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            with connection.cursor() as cursor:
                cursor.execute(
                    "INSERT INTO computer_security_project_secure_customuser "
                    "(is_superuser, is_active, is_staff, failed_login_attempts, username, password, email) "
                    "VALUES (%s, %s, %s, %s, %s, %s, %s)",
                    [False, True, True, 0, form.cleaned_data['username'],
                     make_password(form.cleaned_data['password1']), form.cleaned_data['email']]
                )
...
def login_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            with connection.cursor() as cursor:
                cursor.execute(
                    "SELECT * FROM computer_security_project_secure_customuser "
...
def system_screen(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            with connection.cursor() as cursor:
                cursor.execute(
                    "INSERT INTO computer_security_project_secure_useraccount "
